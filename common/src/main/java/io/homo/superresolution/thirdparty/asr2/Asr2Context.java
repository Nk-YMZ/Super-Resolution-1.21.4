...
import io.homo.superresolution.thirdparty.asr2.struct.Asr2CBFSR2;
import io.homo.superresolution.thirdparty.asr2.struct.Asr2CBRcas;
import io.homo.superresolution.thirdparty.asr2.struct.Asr2CBSpd;
import io.homo.superresolution.thirdparty.fsr2.v233.*;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

public class Asr2Context {
    public static final Logger LOGGER = LoggerFactory.getLogger("SuperResolution-FSR2");

    //////////////////////
    public Asr2Pipeline accumulatePipeline;
    public Asr2Pipeline rcasPipeline;
    public Asr2Pipeline accumulateSharpenPipeline;
    public Asr2Pipeline computeLuminancePyramidPipeline;
    public Asr2Pipeline depthClipPipeline;
    public Asr2Pipeline lockPipeline;
    public Asr2Pipeline reconstructPreviousDepthPipeline;
    //////////////////////

    public Asr2PipelineResources resources = new Asr2PipelineResources();
    public Asr2ContextConfig config;
    public Asr2Dimensions dimensions;
    public Asr2CBFSR2 fsr2Constants = new Asr2CBFSR2();
    public GlBuffer fsr2ConstantsUBO;
    public Asr2CBSpd fsr2SpdConstants = new Asr2CBSpd();
    public GlBuffer fsr2SpdConstantsUBO;
    public Asr2CBRcas fsr2RcasConstants = new Asr2CBRcas();
    ...
    public void resize(Asr2Dimensions dimensions) {
        this.dimensions = dimensions;
        resources.destroy();
        resources = null;
        resources = new Asr2PipelineResources();
        resources.init(
    ...
    public void dispatch(Asr2DispatchDescription dispatchDescription) {
        resources.resource(Asr2PipelineResourceType.INPUT_COLOR).setResource(dispatchDescription.color);
        resources.resource(Asr2PipelineResourceType.INPUT_MOTION_VECTORS).setResource(dispatchDescription.motionVectors);
        resources.resource(Asr2PipelineResourceType.INPUT_DEPTH).setResource(dispatchDescription.depth);
        resources.resource(Asr2PipelineResourceType.INPUT_EXPOSURE).setResource(
                dispatchDescription.exposure == null ?
                        resources.resource(Asr2PipelineResourceType.INTERNAL_DEFAULT_EXPOSURE).getResource() :
                        dispatchDescription.exposure
        );
        resources.resource(Asr2PipelineResourceType.INPUT_REACTIVE_MASK).setResource(
                dispatchDescription.reactive == null ?
                        resources.resource(Asr2PipelineResourceType.INTERNAL_DEFAULT_REACTIVITY).getResource() :
                        dispatchDescription.reactive
        );
        resources.resource(Asr2PipelineResourceType.INPUT_TRANSPARENCY_AND_COMPOSITION_MASK).setResource(
                dispatchDescription.transparencyAndComposition == null ?
                        resources.resource(Asr2PipelineResourceType.INTERNAL_DEFAULT_REACTIVITY).getResource() : //fsr2原版用的也是INTERNAL_DEFAULT_REACTIVITY
                        dispatchDescription.transparencyAndComposition
        );
        resources.resource(Asr2PipelineResourceType.UPSCALED_OUTPUT).setResource(dispatchDescription.output);
        ...
        Asr2PipelineDispatchResource pipelineDispatchResource = new Asr2PipelineDispatchResource(
                resources,
                config,
                dimensions,
                dispatchDescription
