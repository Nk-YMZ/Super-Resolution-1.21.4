...
import io.homo.superresolution.thirdparty.icyllis.modernui.animation.AnimationUtils;
...
public final class Choreographer {
    ...
    private void postCallbackDelayedInternal(int callbackType, @NonNull Object action, @Nullable Object token,
                                             long delayMillis) {
        if (DEBUG_FRAMES) {
            SuperResolution.LOGGER.debug(MARKER, "PostCallback: type=" + callbackType
    ...
    private void removeCallbacksInternal(int callbackType, @Nullable Object action, @Nullable Object token) {
        if (DEBUG_FRAMES) {
            SuperResolution.LOGGER.debug(MARKER, "RemoveCallbacks: type=" + callbackType
                    + ", action=" + action + ", token=" + token);
        }
    ...
    private void scheduleFrameLocked(long now) {
        if (!mFrameScheduled) {
            ...
            if (DEBUG_FRAMES) {
                SuperResolution.LOGGER.debug(MARKER, "Scheduling next frame in " + (nextFrameTime - now) + " ms.");
            }
    ...
    void doFrame() {
        ...
        if (DEBUG_FRAMES) {
            ...
            SuperResolution.LOGGER.info(MARKER, "Frame : Finished, took "
    ...
    void doCallbacks(int callbackType, long frameTimeNanos) {
        ...
        try {
            for (CallbackRecord c = callbacks; c != null; c = c.next) {
                if (DEBUG_FRAMES) {
                    SuperResolution.LOGGER.info(MARKER, "RunCallback: type=" + callbackType
