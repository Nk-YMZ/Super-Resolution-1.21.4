...
public class NativeLibManager {
    ...
    public static NativeApiHelper fsr2api;
    ...
    public static void load(){
        ...
        fsr2api = new NativeApiHelper("N:\\fsr2_opengl_java\\bin\\libfsr2javalib.dll");
        LOGGER.info("FSR2库版本： {}",fsr2api.getVersionInfo());
    }
    ...
    private static boolean extractLibrary(String path, String libraryName) throws IOException {
        Path sourcePath = Paths.get(BASE_PATH, libraryName);
        Path targetPath = Paths.get(path, libraryName);
        try (InputStream in = NativeLibManager.class.getClassLoader().getResourceAsStream(sourcePath.toFile().toString())) {
            if (in == null) {
                return false;
            }
    ...
    public static void main(String[] args) {
        if (NativeLibManager.exists("I:\\superresolution\\fabric\\run")){
        ...
        NativeLibManager.fsr2api.ffxFsr2GetScratchMemorySizeGL();
    }
}
