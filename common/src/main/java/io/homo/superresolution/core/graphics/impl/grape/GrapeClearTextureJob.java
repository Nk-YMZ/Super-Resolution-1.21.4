...
public class GrapeClearTextureJob implements IGrapeJob {
    ...
    public GrapeClearTextureJob clearColor(float r, float g, float b, float a) {
    ...
    public GrapeClearTextureJob clearDepth(float depth) {
        if (depth < 0 || depth > 1) {
    ...
    public GrapeClearTextureJob clearStencil(int stencil) {
    ...
    public GrapeClearTextureJob clearTarget(ITexture target) {
    ...
    public void execute(ICommandBuffer commandBuffer) {
        ...
        if (clearColor != null) {
            ...
            commandBuffer.getDecoder().clearTextureRGBA(commandBuffer, target, clearColor);
            cleared = true;
        }

        if (clearDepth != null) {
            validateDepthSupport();
            commandBuffer.getDecoder().clearTextureDepth(commandBuffer, target, clearDepth);
            cleared = true;
        }

        if (clearStencil != null) {
            validateStencilSupport();
            commandBuffer.getDecoder().clearTextureStencil(commandBuffer, target, clearStencil);
            cleared = true;
        }
