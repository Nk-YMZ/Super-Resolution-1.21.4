...
public class GrapeComputeJob extends GpuComputeJob<GrapeComputeJob> implements IGrapeJob {
    protected Supplier<Vector3i> workGroupSizeSupplier = null;
    protected ComputePipeline pipeline = null;

    public GrapeComputeJob workGroupSize(int x, int y, int z) {
        if (x <= 0 || y <= 0 || z <= 0) {
    ...
    public GrapeComputeJob pipeline(ComputePipeline pipeline) {
        this.pipeline = Objects.requireNonNull(pipeline, "计算管线不能为null");
        return this;
    }

    public GrapeComputeJob workGroupSizeSupplier(Supplier<Vector3i> workGroupSizeSupplier) {
    ...
    public void execute(ICommandBuffer commandBuffer) {
        ...
        Objects.requireNonNull(pipeline, "计算管线未设置");
        ...
        commandBuffer.getDecoder().dispatch(
                commandBuffer,
                pipeline,
                workGroup.x,
                workGroup.y,
