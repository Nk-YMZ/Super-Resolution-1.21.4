...
package io.homo.superresolution.common.minecraft.handler.shadercompat.v2;
...
public class SRCompatConfigV2Parser {
    private static final Gson GSON = new GsonBuilder().create();

    public static SRShaderCompatData parse(JsonObject root) {
        RawSchemaV2 dto = GSON.fromJson(root, RawSchemaV2.class);
        ...
        if (dto.profiles != null) {
            for (Map.Entry<String, RawSchemaV2.RawProfile> entry : dto.profiles.entrySet()) {
                String worldKey = entry.getKey();
                RawSchemaV2.RawProfile rawProfile = entry.getValue();
                if (rawProfile == null) {
                ...
                if (rawProfile.upscale != null && rawProfile.upscale.trigger != null) {
                    RawSchemaV2.RawTrigger rt = rawProfile.upscale.trigger;
                ...
                if (rawProfile.upscale != null) {
                    ...
                    if (rawProfile.upscale.inputs != null) {
                        for (Map.Entry<String, RawSchemaV2.RawInputTexture> inEntry : rawProfile.upscale.inputs.entrySet()) {
                            String inKey = inEntry.getKey();
                            RawSchemaV2.RawInputTexture rit = inEntry.getValue();
                            if (rit == null) {
                    ...
                    if (rawProfile.upscale.outputs != null) {
                        for (Map.Entry<String, RawSchemaV2.RawOutputTexture> outEntry : rawProfile.upscale.outputs.entrySet()) {
                            String outKey = outEntry.getKey();
                            RawSchemaV2.RawOutputTexture rot = outEntry.getValue();
                            if (rot == null) {
                ...
                if (rawProfile.jitter != null) {
                    RawSchemaV2.RawJitter rj = rawProfile.jitter;
                    ...
                    if (rj.source_config != null) {
                        RawSchemaV2.RawJitterSourceConfig rc = rj.source_config;
        ...
        return new SRShaderCompatData(2, profiles, defaultProfile, new SRCompatV2Processor());
    }
    ...
    private static Map<String, SRShaderCompatData.InputTexture> mapInputTextures(Map<String, RawSchemaV2.RawInputTexture> source, String worldKey) {
        Map<String, SRShaderCompatData.InputTexture> result = new HashMap<>();
        if (source == null) return result;
        for (Map.Entry<String, RawSchemaV2.RawInputTexture> e : source.entrySet()) {
            String k = e.getKey();
            RawSchemaV2.RawInputTexture v = e.getValue();
            if (v == null) {
    ...
    private static Map<String, SRShaderCompatData.OutputTexture> mapOutputTextures(Map<String, RawSchemaV2.RawOutputTexture> source, String worldKey) {
        Map<String, SRShaderCompatData.OutputTexture> result = new HashMap<>();
        if (source == null) return result;
        for (Map.Entry<String, RawSchemaV2.RawOutputTexture> e : source.entrySet()) {
            String k = e.getKey();
            RawSchemaV2.RawOutputTexture v = e.getValue();
            if (v == null) {
    ...
    private static boolean validateRawSourceConfig(RawSchemaV2.RawSourceConfig rsc, String context) {
        if (rsc == null) {
    ...
    private static class RawSchemaV2 {
        int schema_version;
