...
extern "C" {
    ...
    NVGcontext *nvgCreateInternal(NVGparams *params) {
        ...
        // Create font texture
        ctx->fontImages[0] = ctx->params.renderCreateTexture(ctx->params.userPtr, NVG_TEXTURE_ALPHA, fontParams.width,
    ...
    void nvgRoundedRectVarying(NVGcontext *ctx, float x, float y, float w, float h, float radTopLeft, float radTopRight,
        ...
        if (radTopLeft < 0.1f && radTopRight < 0.1f && radBottomRight < 0.1f && radBottomLeft < 0.1f) {
            ...
            float rxTL = nvg__minf(radTopLeft, halfw) * nvg__signf(w), ryTL =
                    nvg__minf(radTopLeft, halfh) * nvg__signf(h);
            float vals[] = {
                NVG_MOVETO, x, y + ryTL,
    ...
    void nvgRoundedRectEllipse(NVGcontext *ctx, float x, float y, float w, float h, float rw, float rh) {
        if (rw < 0.1f || rh < 0.1f) {
            nvgRect(ctx, x, y, w, h);
            return;
        } else {
            float vals[] = {
                NVG_MOVETO, x + rw, y,
                NVG_LINETO, x + w - rw, y,
                NVG_BEZIERTO, x + w - rw * (1 - NVG_KAPPA90), y, x + w, y + rh * (1 - NVG_KAPPA90), x + w, y + rh,
                NVG_LINETO, x + w, y + h - rh,
                NVG_BEZIERTO, x + w, y + h - rh * (1 - NVG_KAPPA90), x + w - rw * (1 - NVG_KAPPA90), y + h, x + w - rw,
                y + h,
                NVG_LINETO, x + rw, y + h,
                NVG_BEZIERTO, x + rw * (1 - NVG_KAPPA90), y + h, x, y + h - rh * (1 - NVG_KAPPA90), x, y + h - rh,
                NVG_LINETO, x, y + rh,
                NVG_BEZIERTO, x, y + rh * (1 - NVG_KAPPA90), x + rw * (1 - NVG_KAPPA90), y, x + rw, y,
                NVG_CLOSE
            };
            nvg__appendCommands(ctx, vals, NVG_COUNTOF(vals));
        }
    ...
    void nvgRoundedRectComplex(NVGcontext *ctx, float x, float y, float w, float h, float tl, float tr, float bl,
                               float br) {
        if (tl < 0.1f && tr < 0.1f && bl < 0.1f && br < 0.1f) {
            nvgRect(ctx, x, y, w, h);
            return;
        } else {
            float vals[] = {
                NVG_MOVETO, x + tl, y,
                NVG_LINETO, x + w - tr, y,
                NVG_BEZIERTO, x + w - tr * (1 - NVG_KAPPA90), y, x + w, y + tr * (1 - NVG_KAPPA90), x + w, y + tr,
                NVG_LINETO, x + w, y + h - br,
                NVG_BEZIERTO, x + w, y + h - br * (1 - NVG_KAPPA90), x + w - br * (1 - NVG_KAPPA90), y + h, x + w - br,
                y + h,
                NVG_LINETO, x + bl, y + h,
                NVG_BEZIERTO, x + bl * (1 - NVG_KAPPA90), y + h, x, y + h - bl * (1 - NVG_KAPPA90), x, y + h - bl,
                NVG_LINETO, x, y + tl,
                NVG_BEZIERTO, x, y + tl * (1 - NVG_KAPPA90), x + tl * (1 - NVG_KAPPA90), y, x + tl, y,
                NVG_CLOSE
            };
            nvg__appendCommands(ctx, vals, NVG_COUNTOF(vals));
        }
    ...
    int nvg__textBreakLines(NVGcontext *ctx, const char *string, const char *end, float breakRowWidth, NVGtextRow *rows,
        ...
        while (fonsTextIterNext(ctx->fs, &iter, &q)) {
            ...
            if (type == NVG_NEWLINE) {
                ...
                rows[nrows].start = (char *) (rowStart != NULL ? rowStart : iter.str);
                rows[nrows].end = (char *) (rowEnd != NULL ? rowEnd : iter.str);
                rows[nrows].width = rowWidth * invscale;
                rows[nrows].minx = rowMinX * invscale;
                rows[nrows].maxx = rowMaxX * invscale;
                rows[nrows].next = (char *) (iter.next);
                nrows++;
                if (nrows >= maxRows)
                    return nrows;
                ...
                if (rowStart == NULL) {
                    ...
                    if ((type == NVG_CHAR || type == NVG_CJK_CHAR) && nextWidth > breakRowWidth) {
                        ...
                        if (breakEnd == rowStart) {
                            ...
                            rows[nrows].start = (char *) (rowStart);
                            rows[nrows].end = (char *) (iter.str);
                            rows[nrows].width = rowWidth * invscale;
                            rows[nrows].minx = rowMinX * invscale;
                            rows[nrows].maxx = rowMaxX * invscale;
                            rows[nrows].next = (char *) (iter.str);
                            nrows++;
                            if (nrows >= maxRows)
                                return nrows;
                            rowStartX = iter.x;
                            ...
                            rows[nrows].start = (char *) (rowStart);
                            rows[nrows].end = (char *) (breakEnd);
                            rows[nrows].width = breakWidth * invscale;
                            rows[nrows].minx = rowMinX * invscale;
                            rows[nrows].maxx = breakMaxX * invscale;
                            rows[nrows].next = (char *) (wordStart);
                            nrows++;
                            if (nrows >= maxRows)
                                return nrows;
                            // Update row
        ...
        if (rowStart != NULL) {
            rows[nrows].start = (char *) (rowStart);
            rows[nrows].end = (char *) (rowEnd);
            rows[nrows].width = rowWidth * invscale;
            rows[nrows].minx = rowMinX * invscale;
            rows[nrows].maxx = rowMaxX * invscale;
            rows[nrows].next = (char *) (end);
            nrows++;
        }
    ...
    void nvgTextBox(NVGcontext *ctx, float x, float y, float breakRowWidth, const char *string, const char *end) {
        ...
        while ((nrows = nvg__textBreakLines(ctx, string, end, breakRowWidth, rows, 2))) {
            for (i = 0; i < nrows; i++) {
    ...
    int nvgTextGlyphPositions(NVGcontext *ctx, float x, float y, const char *string, const char *end,
        ...
        while (fonsTextIterNext(ctx->fs, &iter, &q)) {
            ...
            prevIter = iter;
            positions[npos].str = (char *) (iter.str);
...
std::vector<NVGtextRow> nvgTextBreakLines(NVGcontext *ctx, const char *string, const char *end, float breakRowWidth) {
    ...
    while (rowStart && (end == nullptr || rowStart < end)) {
        NVGtextRow row;
        int nrows = nvg__textBreakLines(ctx, rowStart, end, breakRowWidth, &row, 1);
        ...
        if (rowStart == nullptr || (end && rowStart >= end)) {
            break;
        }
    }
