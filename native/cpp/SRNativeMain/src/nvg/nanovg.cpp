...
extern "C"
{
	...
	void nvgRoundedRectEllipse(NVGcontext *ctx, float x, float y, float w, float h, float rw, float rh)
	{
		...
		else
		{
			float vals[] = {
				...
				NVG_BEZIERTO, x + w, y + h - rh * (1 - NVG_KAPPA90), x + w - rw * (1 - NVG_KAPPA90), y + h, x + w - rw, y + h,
				NVG_LINETO, x + rw, y + h,
	...
	void nvgRoundedRectComplex(NVGcontext *ctx, float x, float y, float w, float h, float tl, float tr, float bl, float br)
	{
		...
		else
		{
			float vals[] = {
				...
				NVG_BEZIERTO, x + w, y + h - br * (1 - NVG_KAPPA90), x + w - br * (1 - NVG_KAPPA90), y + h, x + w - br, y + h,
				NVG_LINETO, x + bl, y + h,
	...
	int nvg__textBreakLines(NVGcontext *ctx, const char *string, const char *end, float breakRowWidth, NVGtextRow *rows, int maxRows)
	{
		NVGstate *state = nvg__getState(ctx);
		float scale = nvg__getFontScale(state) * ctx->devicePxRatio;
		float invscale = 1.0f / scale;
